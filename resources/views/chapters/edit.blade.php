  ...
  <div class="container mx-auto p-8 mt-28 bg-white rounded shadow flex items-center justify-center">
      <div class="py-4  px-6">
        <div class="flex justify-between items-center pb-3">
          <p class="text-2xl font-bold">Modifier un chapitre</p>
        </div>

        <form action="{{ route('chapters.update', $chapter) }}" method="POST" enctype="multipart/form-data">
          @csrf
          @method('PUT')
          <input type="text" name="title" class="border border-gray-400 px-2 py-2 focus:outline-none w-full @error('title') border border-red-500 @enderror"  value="{{ $chapter->title }}">
          <input type="hidden" name="module_id" value="{{ $chapter->module->id }}">
          ...
          <span class="file-chosen">{{ $chapter->file }}</span>
