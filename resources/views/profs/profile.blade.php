...
<section class="container mx-auto mt-28 px-8 mb-10">
  ...
  <div class="grid md:grid-cols-2 gap-4">
    <div class="flex flex-col md:flex-row items-center bg-white rounded shadow p-8 mt-4">
      <img src="{{ asset('/files/avatars/' . $prof->avatar) }}" alt="photo de profile" class="rounded-full w-60 h-60">
      <div class="border-l-4 border-gray-800 ml-6 pl-4 mt-4">
        <h4 class="text-gray-700 uppercase font-bold text-2xl">{{ $prof->name }}</h1>
        <h5 class="text-gray-700 uppercase font-bold">Filière : @foreach($prof->modules as $module) {{ $module->filiere->name }} @endforeach</h5>
        <h5 class="text-gray-700 uppercase font-bold">Modules : {{ $prof->modules->count() }}</h5>
        <h5 class="text-gray-700 uppercase font-bold">Discussions : {{ $prof->discussions->count() }} </h5>
      </div>
    </div>
    <div class="flex items-center flex-col md:flex-row bg-white rounded shadow p-8 mt-4">
      <form class="w-full max-w-lg" action="{{ route('profs.updateProfile') }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="flex flex-wrap -mx-3 mb-6">
          <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
            ...
            <input name="name" class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" type="text" value="{{ $prof->name }}">
          </div>
          <div class="w-full md:w-1/2 px-3">
            <label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" >
              Adresse E-mail
            </label>
            <input name="email" class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" type="text" value="{{ $prof->email }}">
          </div>
        </div>
        <div class="flex flex-wrap -mx-3 mb-6">
          <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
