...
<section class="container mx-auto mt-28 px-4 mb-10">
  <div class="flex flex-col md:flex-row justify-between">
    <a href="{{ route('admin.profs') }}"><h1 class="text-3xl font-semibold text-green-500"><i class="fas fa-user-tie"></i> LES PROFESSEURS</h1></a>
    <form class="w-full max-w-md" action="{{ route('profs.search') }}" method="GET">
      @csrf
      <div class="flex items-center mt-4 md:mt-0">
        ...
        <button type="submit" class="flex-shrink-0 bg-blue-500 hover:bg-blue-700 border-blue-500 uppercase hover:border-blue-700 text-sm border-4 text-white py-1 px-2 rounded">
          <i class="fas fa-search"></i> Rechercher Professeur
        </button>
      </div>
    </form>
  </div>
  <div class="grid md:grid-cols-2 mt-10 gap-4">
    ...
    <div>
      <table class="table-auto">
        ...
        <tbody>
          ...
          <tr>
            <td class="border border-green-500 px-4 py-2 text-gray-600 font-bold"> <img src="{{ asset('files/avatars/' . $prof->avatar) }}" alt="photo de profile" class="rounded w-12 h-12"> </td>
            <td class="border border-green-500 px-4 py-2 text-gray-600 font-bold"> {{ $prof->name }} </td>
            <td class="border border-green-500 px-4 py-2 text-gray-600 font-bold"> {{ $prof->email }} </td>
            <td class="border border-green-500 px-4 py-2 text-gray-600 font-medium">
              <div class="flex justify-center">
                <div>
                  <a href="{{ route('professors.edit', $prof) }}" class="hover:text-yellow-600"> <i class="fas fa-edit fa-lg mr-2"></i> </a>
                </div>
                    ...
                    <div class="h-auto p-4 text-left bg-white rounded shadow-xl md:max-w-xl md:p-6 lg:p-8 md:mx-0" @click.away="open = false">
                      <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                        <h3 class="text-2xl font-medium text-gray-900">
                          Supprission d'un professeur
                        </h3>

                        <div class="mt-2">
                          <p class="text-gray-800">
                            Etes-vous sûr de vouloir supprimer ce professeur ?
                          </p>
                      </div>
                    </div>
                      ...
                      <div class="mt-5 sm:mt-6">
                        <span class="flex justify-start">
                          <form action="{{ route('professors.destroy', $prof) }}" method="post">
                            @csrf
                            @method('DELETE')
    ...
    <div class="mt-6 md:mt-0">
      <h1 class="uppercase text-blue-700 font-bold">Ajouter un nouveau professeur</h1>
      <div class="flex items-center flex-col md:flex-row bg-white rounded shadow p-8 mt-4">
        <form class="w-full max-w-lg" action="{{ route('professors.store') }}" method="POST">
          @csrf
          ...
          <button type="submit" class="p-2 uppercase px-2 text-white bg-blue-500 hover:bg-blue-800 rounded font-medium focus:outline-none" name="button">Ajouter Professeur</button>
        </form>
      </div>
    </div>
  </div>
